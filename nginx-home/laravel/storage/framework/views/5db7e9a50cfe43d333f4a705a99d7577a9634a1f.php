...
<form class="" role="form" id="orderform" name="orderform">
	...
	<div class="form-group shadedform row">
		...
		<label for="patient_sex" class ="control-label">Sex:<i class="fas fa-asterisk"></i></label>
            <?php Log::info($patient->sex); ?>
			<select class="SumoUnder" id="patient_sex" name="patient_sex" required readonly>
				<option disabled selected>
					Select option
				</option>

				<option value="M" <?php ($patient->sex == 'M')?"selected":"" ?> >
					M
				</option>
				<option value="F" <?php ($patient->sex == 'F')?"selected":"" ?> >
					F
				</option>
				<option value="O" <?php ($patient->sex == 'O')?"selected":"" ?> >
					O
				</option>
			</select>
    ...
    <div class="form-group shadedform row">
		...
		<label for="accession_number" class ="control-label">Appt. ID:</label>
	      <input  readonly type="text" class="form-control" id="appointment_id" name="appointment_id" value="<?php echo $result ?>" placeholder = "System Generated" >
		</div>
    ...
    <div class="form-group shadedform row">
		...
		<div class="col-sm-4">
			...
			$result = DatabaseFactory::getReferrersSelectList($order->referring_physician_id); // just gets them all
			Log::info($result);
            echo $result;
            ?>
            </select><br>
...
?>
<?php /**PATH /nginx-home/laravel/resources/views/orders/orderform.blade.php ENDPATH**/ ?>
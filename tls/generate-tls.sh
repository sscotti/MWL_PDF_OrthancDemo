...
openssl req -new -sha256 -config nginx.cnf -key nginx-key.pem -out nginx-csr.pem

# now let's sign the requests with our CA
openssl x509 -req -sha256 -extfile nginx.cnf -days 999 -passin "pass:password" -in nginx-csr.pem -CA ca-crt.pem -CAkey ca-key.pem -CAcreateserial -out nginx-crt.pem
